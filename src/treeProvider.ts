...
function colorDotIcon(hex: string): vscode.Uri {
  const safe = /^#[0-9a-fA-F]{6}$/.test(hex) ? hex : '#808080';
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16"><circle cx="8" cy="8" r="6" fill="${safe}"/></svg>`;
  return vscode.Uri.parse(`data:image/svg+xml;utf8,${encodeURIComponent(svg)}`);
}

function colorFolderIcon(hex: string): vscode.Uri {
  const safe = /^#[0-9a-fA-F]{6}$/.test(hex) ? hex : '#808080';
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16"><path d="M1.5 3.5A1 1 0 0 1 2.5 2.5H6l1.5 1.5H13.5A1 1 0 0 1 14.5 5V12A1 1 0 0 1 13.5 13H2.5A1 1 0 0 1 1.5 12Z" fill="${safe}"/></svg>`;
  return vscode.Uri.parse(`data:image/svg+xml;utf8,${encodeURIComponent(svg)}`);
}
  ...
  implements vscode.TreeDataProvider<TreeNode>, vscode.TreeDragAndDropController<TreeNode>
{
  ...
  private groupToItem(group: Group): vscode.TreeItem {
    ...
    item.iconPath = group.color ? colorFolderIcon(group.color) : new vscode.ThemeIcon('folder');
    return item;
  }
